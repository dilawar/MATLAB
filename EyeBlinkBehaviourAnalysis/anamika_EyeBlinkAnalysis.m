...
% This code additionally requires getInfo.m and blinkFilter.m to work.
...
while 1
    ...
    if str2num(sessionType) <= 5
        totalTrials = 40;
        if str2num(sessionType) == 3
            traceTime = 250; %in ms
        elseif str2num(sessionType) == 5
            traceTime = 500; %in ms
        else
        ...
        totalTrials = 100;
        if str2num(sessionType) == 9
            traceTime = 250; %in ms
        elseif str2num(sessionType) == 11
            traceTime = 500; %in ms
        else
            warning('Unable to determine SessionType');
        end
    ...
    blinkData = zeros(totalTrials, min(rawData_size)); %to store raw data
    blinkData_dRR = zeros(totalTrials, min(rawData_size)); %to store baseline corrected data (Percent dR/R)
    ...
    blinkData_csPlus = [];
    blinkData_csMinus = [];
    blinkData_csPlus_dRR = [];
    blinkData_csMinus_dRR = [];
    ...
    for trialNum = 1:totalTrials
        ...
        if doBaselineCorrection == 1
            ...
            for sample = 1: size(blinkData,2)
                blinkData_dRR(trialNum, sample) = ((blinkData(trialNum,sample)/blinkData_baselines(trialNum,1))-1)*100; %baseline corrected
            end
            clear sample
        end
        ...
        if splitCS == 1
            if csType(trialNum) == 1
                ...
                blinkData_csPlus_dRR = [blinkData_csPlus_dRR; blinkData_dRR(trialNum,:)];
                if filterData ==1
                ...
                try
                    if csvread(filename, nHeaderLines+1, 3, [ nHeaderLines+1 3 nHeaderLines+1 3] ) == 0   %reads only the row after nHeaderLines, column 4
                        probeTrials = [probeTrials size(blinkData_csPlus_dRR,1)];
            ...
            elseif csType(trialNum) == 0
                ...
                blinkData_csMinus_dRR = [blinkData_csMinus_dRR; blinkData_dRR(trialNum,:)];
                if filterData == 1
    ...
    if saveData == 1
        ...
        csvwrite([saveFolder, dataset, '_csPlus_dRR.csv'], blinkData_csPlus_dRR);
        csvwrite([saveFolder, dataset, '_csMinus_dRR.csv'], blinkData_csMinus_dRR);
        ...
        %baseline corrected data
        save([saveFolder dataset '_csPlus_dRR'], 'blinkData_csPlus_dRR');
        save([saveFolder dataset '_csMinus_dRR'], 'blinkData_csMinus_dRR');
        % CS Type for the session
