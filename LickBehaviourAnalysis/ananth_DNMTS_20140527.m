...
while 1 %You might see a variable "ans" = 0, after the program runs to completion
    ...
    session=dataset((uscorei(1,2)+1):length(dataset)); %Session
    ...
    results_path=[direc mouse '_' task '_' session '_result.m'];
...
%     2. Pre-Tone Lick
%     3. Tone 1 Lick
%     4. Delay Interval Lick
%     5. Tone 2 Lick
%     6. Pre-Tone Duration (ms)
%     7. Tone 1 Duration (ms)
%     8. Delay Interval Duration (ms)
%     9. Tone 2 Duration (ms)
%     10. Trial Condition (1 = Non-Match Trials; 0 = Match Trials)
%     11. Success Rate (%)
%     12. Failure Rate (%)
%     13. Performance (%)
%     14. Lick Dependency (1 = Yes; 0 = No)
%     15. Correct Licks
%     16. Incorrect Licks
%     17. Aborted Trials
    ...
    CorrectTrials=(results(ntrials, 15)); %(the summing bit happens in the behaviour code itself)
    nNMTs=sum(results(:,10)); %number of Non-Match Trials (sums all the "1"s in the column designated to "Trial Condition")
    SuccessRate(sessionnumber)=(CorrectTrials/nNMTs)*100; %in "%" for all sessions considered
    
    IncorrectTrials=(results(ntrials,16)); %(the summing bit happens in the behaviour code itself)
    nMTs=ntrials-(sum(results(:,10))); %number of Match Trials (Effectively, sums all the "0"s in the column designated to "Trial Condition")
