...
dataset_date=('20140130');
dataset_mousename=('test2');
dataset_ROIindex=('red_wPMT-ROI');
...
tifffilename='1_Trial-1_ROI-1.tif';
...
tiftag = imfinfo([direc tifffilename]); %use the tiff file to be used to make cellmask
img_desc = tiftag.ImageDescription;
%
%     framei = findstr(img_desc, 'Frames');
nframes = str2num(img_desc(43:46)); % @! Always edit to reflect the differences in tiftag for SkullVEIW and SenseVIEW
clear framei;
...
imagei = double(imread([direc tifffilename], 1));
for i = 2:5
    image = double(imread([direc tifffilename], i));
    imagei = imagei + image;
end
...
BW=zeros(second,first); % @! Always edit for every ROI dataset - reverse the order of the resolution listing given by (Fiji is just) ImageJ
...
for trial = 1 : ntrials
    ...
    for count = 1 : nframes
        tempimg = imread([direc tifffilename], count);
