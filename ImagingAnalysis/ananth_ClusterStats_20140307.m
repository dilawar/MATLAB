% Perform an analysis of the Intra- and Inter-Cluster Correlation
% Coefficients, for all datasets, defined by i
...
%load('/Users/ananth/Desktop/Work/Analysis/ImageAnalysis/20131218/mouse4/Tone10knPuff-ROI/cal.mat')
%load('/Users/ananth/Desktop/Work/Analysis/ImageAnalysis/20131218/mouse4/Tone10knPuff-ROI/cellmask.mat')
%load('/Users/ananth/Desktop/Work/Analysis/ImageAnalysis/20131218/mouse4/Tone10knPuff-ROI/calbdf.mat')
%load('/Users/ananth/Desktop/Work/Analysis/ImageAnalysis/20131218/mouse4/Tone10knPuff-ROI/caltr.mat')
...
%     load([num2str(i) '_cellmask.mat']);
    ...
    for p = size(a, 1)
        ...
        % Nearest neghbour cluster identities and correlations
        for q = 1 : length(temp)
        ...
        % Cluster maps
        cellmaskc = cellmask*0;
        for j = 1 : size(a, 1)
            temp = a{j, 1};
            for x = 1 : length(temp)
                cellmaskc(cellmask == (temp(x))) = j;
            end
        end
        %figure; imagesc(cellmaskc)
    end
    
    %figure(1); hold on
    errorbar([0 1], [mean(ciclc) mean(coclc)], [(std(ciclc)/sqrt(length(ciclc))) (std(ciclc)/sqrt(length(ciclc)))], 'k', 'LineWidth', 3);
    title('Intra- and Inter-cluster Correlation Coefficients', 'fontsize', 24, 'fontweight', 'b')
    ylabel('Mean Correlation Coefficient', 'fontsize', 20, 'fontweight', 'b')
    set(gca, 'XTick', (0:1));
    set(gca, 'XTickLabel', {'Intracluster','Intracluster'});
end
